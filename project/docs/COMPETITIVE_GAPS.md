...
|---|---|---|
| 46 | **Data augmentation** — SpecAugment, speed/pitch perturbation, noise/RIR mixing (NeMo, SpeechBrain) | The single biggest accuracy win on small kid datasets; the director should auto-enable it for tiny data. |
| 47 | **Dataset import** — folder of (audio, transcript) pairs, NeMo/JSON manifest, Common Voice TSV, LibriSpeech, HF `datasets` (all three) | Lets a grown-up bring an existing corpus instead of only recording in-app. |
...
| 50 | **Decoding controls** — beam size, `initial_prompt`/hotword biasing, temperature fallback (faster-whisper, NeMo) | Cheap accuracy + lets a child bias toward their vocabulary. |
| 51 | **Punctuation + capitalization restoration / ITN** (NeMo) | Readable transcripts; big perceived-quality win. |
| 52 | **Richer evaluation** — per-utterance WER, a confusion/error report, confidence, normalized vs raw WER (all three) | Powers active learning (#32) and Grown-up insight beyond one number. |
| 53 | **Local experiment metrics view** — TensorBoard-style loss/WER curves, on-device only (HF, NeMo, SpeechBrain) | Grown-up mode depth without telemetry (honours D-008). |
...
| 56 | **Optional multi-GPU / distributed** training (all three) | Out of scope for a family laptop, but a documented escape hatch for power users. |
| 57 | **More export targets** — HF `safetensors`, GGUF (whisper.cpp), TorchScript (HF, llama.cpp ecosystem) | Interop with other runtimes the family may already use. |
