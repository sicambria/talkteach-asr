...
def test_perturb_pitch_preserves_length():
    x = _tone(1.0)
    up = perturb_pitch(x, 2.0)
    assert up.shape == x.shape  # duration preserved
    assert not np.allclose(up, x)  # but signal changed
    assert np.allclose(perturb_pitch(x, 0.0), x)  # no-op
