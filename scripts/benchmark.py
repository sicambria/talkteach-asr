...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--config", required=True, help="benchmark config (.yaml/.yml/.json)")
    ap.add_argument("--out", default=None, help="write JSON results here (default: under --workdir)")
    ap.add_argument(
        "--report", default=None, help="write the Markdown scoreboard report here (default: under --workdir)"
    )
    ap.add_argument(
        "--workdir", default=None, help="scratch dir for audio + checkpoints (default: temp)"
    )
    ap.add_argument("--train-clips", type=int, default=None, help="override config train_clips")
    ap.add_argument("--eval-clips", type=int, default=None, help="override config eval_clips")
    ...
    # Present: the full matrix, then the ELO-ranked scoreboard.
    print(format_table(report))
    print("\n" + format_scoreboard(report))
    ...
    write_markdown(report, md, generated_at=time.strftime("%Y-%m-%d %H:%M:%S %Z"))
    ...
    if report.cells and all(c.status != "ok" for c in report.cells):
        print("WARNING: no cell completed successfully (deps/binaries missing?)", file=sys.stderr)
        return 2
    return 0
