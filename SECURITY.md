...
> telemetry without explicit opt-in** (see `DECISIONS.md` D-008). A bug that
...
- **Path traversal / arbitrary file write.** Uploads and project paths must stay
  ...
  allow-list (`DECISIONS.md` D-004); regressions in that boundary are in scope.
...
- **The Tauri Content-Security-Policy.** The shipped CSP is locked to `'self'`
  ...
  progress) — see `DECISIONS.md` D-005. Any way to bypass or weaken it, or to
...
- **Upload validation.** Size limits and the codec/extension allow-list that
  guard the analyze/upload endpoints (roadmap #9; see `DECISIONS.md` D-004).
